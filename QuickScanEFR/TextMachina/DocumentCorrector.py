...
class EFRCorrector:
    ...
    def compute_missing_vems(self):
        ...
        vems_percent_values = [float(str(val).replace('%', '')) / 100 if pd.notnull(val) else None for val in vems_percent_row.iloc[0, 1:].values]
        ...
        for idx, (vems, vems_percent) in enumerate(zip(vems_values, vems_percent_values)):
            if pd.isnull(vems) and vems_percent is not None:
                ...
                current_date_year = pd.to_datetime(dates[idx]).year
                if 0 <= left_idx < len(vems_values):
                    left_date_year = pd.to_datetime(dates[left_idx]).year
                else:
                    left_date_year = None
                    
                if 0 <= right_idx < len(vems_values):
                    right_date_year = pd.to_datetime(dates[right_idx]).year
                else:
                    right_date_year = None

                # If either left or right VEMS value is found and its date is in the same year, extrapolate VEMS
                if (0 <= left_idx < len(vems_values) and left_date_year == current_date_year) or (0 <= right_idx < len(vems_values) and right_date_year == current_date_year):
                    if left_date_year == current_date_year and right_date_year == current_date_year:
                    ...
                    elif left_date_year == current_date_year:
                        vems_estimated = vems_values[left_idx] * (vems_percent / vems_percent_values[left_idx])
                    else:
                        vems_estimated = vems_values[right_idx] * (vems_percent / vems_percent_values[right_idx])
    ...
    def save_combined_data(self, output_file_path=None):
        ...
        if output_file_path is None:
            output_file_path = self.excel_file_path.replace(".xlsx", "_corrected.xlsx")
    ...
    def correct_multiple_docs(self, directory_path):
        ...
        """
        excel_files = [f for f in os.listdir(directory_path) if f.endswith('_cleaned_modified.xlsx')]
